...
namespace Hooks
{
	...
	void Initialize()
	{
		...
		netgraphtext_hook.setup(Interfaces::Convar->FindVar(("net_graphtext")));
		sv_cheats.setup(Interfaces::Convar->FindVar(("sv_cheats")));
	...
	bool __fastcall hkNetGraph(void* ecx, void* edx)
	{
		...
		static auto v1 = (DWORD)Utils::PatternScan(GetModuleHandleA(("client.dll")), ("85 C0 0F 84 ? ? ? ? A1 ? ? ? ? 0F 57 C0 F3 0F 10 48"));
		static auto cnetgraph = **(uintptr_t**)(Utils::PatternScan(GetModuleHandleA(("client.dll")), ("89 1D ? ? ? ? 8B C3 5B 8B E5 5D C2 04")) + 2);
	...
	void __stdcall hkPaintTraverse(vgui::VPANEL panel, bool forceRepaint, bool allowForce)
	{
		...
		if (Globals::LocalPlayer && Globals::LocalPlayer->IsAlive() && Vars.visuals_noscopeborder && !strcmp(("HudZoom"), panelName))
			return;
	...
	bool __fastcall hkFireEventClientSideThink(void* ecx, void* edx, IGameEvent* pEvent)
	{
		...
		{
			ConVar* sv_skyname = Interfaces::Convar->FindVar(("sv_skyname"));
	...
	static auto dwCAM_Think = Utils::PatternScan(GetModuleHandleA(("client.dll")), ("85 C0 75 30 38 86"));
