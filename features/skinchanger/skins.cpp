		...
		return;
	{
		...
		if (glove_config && glove_config->definition_override_index)
		{
			if (!glove)
			{
				...
				assert(glove);
				{
					static auto set_abs_origin_addr = Utils::PatternScan(GetModuleHandleA(("client.dll")), ("55 8B EC 83 E4 F8 51 53 56 57 8B F1"));
...
auto CSkinChanger::Initialize() -> void
{
	...
	const auto sig_address = Utils::PatternScan(GetModuleHandleW(L"client.dll"), "E8 ? ? ? ? FF 76 0C 8D 48 04 E8");
